...
def handle_connect():
       if 'user' in session:
           ...
           app.logger.info(f"User connected: {username}, SID: {request.sid}")
           
           print(f'User connected: {username }')
           
           with engine.connect() as conn:
           	result = conn.execute(text("""SELECT id, username, message, timestamp, message_type FROM messages ORDER BY timestamp"""))
