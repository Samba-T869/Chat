...
@app.before_request
def block_scanners():
    blocked_paths = ['/license.txt', '/.env', '/admin']
    if any(path in request.path for path in blocked_paths):
        abort(404)
    
    # Block suspicious user agents
    ua = request.headers.get('User-Agent', '')
    bad_agents = ['nmap', 'sqlmap', 'dirb']
    if any(agent in ua.lower() for agent in bad_agents):
        abort(403)
...
def handle_public_message(data):
    ...
    with sqlite3.connect('users.db') as conn:
        cur = conn.cursor()
        cur.execute('''
            INSERT INTO public_messages(sender_username, recipient_username, message_text, file_path, file_type, timestamp) 
            VALUES(?,?,?,?,?,?)
        ''', (sender_username, 'public', message_text, file_path, file_type, timestamp))
        conn.commit()
    
    cur.execute('SELECT timestamp FROM public_messages WHERE id = last_insert_rowid()')
